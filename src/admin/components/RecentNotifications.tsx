...
const RecentNotifications = () => {
  ...
  const unreadCount = useMemo(() => data && data.filter((notification) => notification.unread).length, [data]);
    ...
    <Box>
      ...
      >
        <Box sx={{ width: 360 }}>
              ...
              {data.map((notification) => (
                <ListItemButton component={NavLink} key={notification.id} to={''}>
                  ...
                  <ListItemText
                    ...
                    secondary={formatDistanceToNow(new Date(notification.createdAt), { addSuffix: true, locale })}
                  />
                </ListItemButton>
              ))}
            </List>
          ...
          {isLoading && <Loader />}
          <Box sx={{ px: 2, pb: 2 }}>
            <Button color='secondary' fullWidth sx={{ bgcolor: 'background.default' }} variant='contained'>
